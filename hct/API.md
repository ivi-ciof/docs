# CIOF システムAPIリファレンスマニュアル v2.3.4
...
### 2.7.13 データファイルメタデータの取得

本APIを用いて、データファイルの送信APIでアップロードしたファイルのメタデータを取得することができます。
アップロード後のファイルのステータス（`pending` / `uploaded` / `failed`）を確認したり、ファイル名・サイズ・チェックサムなどの情報を取得したりする際に利用します。
指定するファイルIDは、データファイルの送信APIの応答として返された `id` を使用してください。
取得できるのは、リクエストを行ったコントローラ自身がアップロードしたファイルのメタデータに限られます。

- **HTTPメソッド**: GET
- **URLパス**: `/hct/api/v2/blobs/:blob_id`
- **要求ヘッダ**: [^4]
  ```
  Authorization: Bearer token=<authorization_key>
  ```
- **要求パラメータ**: URLパスにパラメータとして、以下を指定します。
  ...
  | 属性 | 型 | 常に存在 | 説明 |
  | --- | --- | --- | --- |
  | blob_id | string | ✓ | URLパスに含めるファイルID。`blob:6cf4ad3c-8b76-494f-9250-befe240bd870` 形式。 |
- **応答ステータスコード**: [2.3節](#23-httpステータスコード)に準じます。

  | ステータスコード | 内容 |
  | --- | --- |
  | 200 | メタデータを正常に取得しました。 |
  | 404 | 指定されたIDのファイルが存在しないか、アクセス権がありません。 |
- **応答ヘッダ**:
  ```
  Content-Type: application/json
  ```
- **応答パラメータ**: メッセージボディ部に、以下のパラメータを返します。
  ```json
  {
    "id": "blob:6cf4ad3c-8b76-494f-9250-befe240bd870",
    "status": "uploaded",
    "byte_size": 12331250,
    "filename": "image.jpg",
    "checksum": "Ej8HXZQF3TbovziAnPol9w==",
    "inserted_at": "2025-08-29 12:38:37",
    "uploaded_at": "2025-08-29 12:38:37",
  ...
  | --- | --- | --- | --- |
  | id | string | ✓ | ファイルのID。このIDを取引データの送信APIにおいて、message送信のパラメータに設定します。 |
  | status | string | ✓ | ファイルのアップロード状態。`pending`（アップロード待ち）、`uploaded`（アップロード済み）、`failed`（失敗）のいずれか。 |
  | byte_size | integer | ✓ | ファイルのサイズ（バイト単位）。 |
  | filename | string | ✓ | ファイル名。 |
  | checksum | string | ✓ | ファイルのチェックサム。ファイルをMD5でハッシュしてBase64にしたものです。 |
  | inserted_at | string | ✓ | ファイルがシステムに登録された日時。 |
  | uploaded_at | string | | ファイルのアップロードが完了した日時。アップロード前は `null`。 |
  | content_type | string | ✓ | ファイルのMIMEタイプ（例: `image/jpeg`）。 |
...
| 2.3.4 | 2026.04.09 | IVI 日名 | 2.7.13 データファイルメタデータの取得 APIを追加<br>2.7.13〜2.7.14 の章番号を 2.7.14〜2.7.15 に振り直し |
